...
import { cancelAgentTurn, startAgentTurnStream } from "@/lib/web-agent"
...
export function ChatPanel() {
    ...
    async (
      ...
      try {
        ...
        if (useBackendAgent) {
          ...
          try {
            await startAgentTurnStream(project?.id ?? "current", {
              message: text,
              sessionId: convId,
              runId: backendRunId,
  ...
  const handleStop = useCallback(() => {
    ...
    if (sessionId) {
      void cancelAgentTurn(project?.id ?? "current", sessionId).catch(() => {})
    }
